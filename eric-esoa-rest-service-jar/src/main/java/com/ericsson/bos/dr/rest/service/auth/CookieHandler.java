...
import static com.ericsson.bos.dr.rest.service.connectivity.Subsystem.AUTH_TYPE_COOKIE;
...
/**
 * Cookie auth token handler. Retrieves a token of 'auth.type' Cookie to allow further access to an external system.
 *
 * The <code>CookieHandler</code> makes a request to a token endpoint ('auth.url') of the external system.
 * It then retrieves the token of 'auth.type' Cookie from a Set-Cookie response header, using the 'auth.tokenRef' property.
 * The token will be returned inside the <code>TokenData</code> and added to the TokenCache.
 *
 * If a matching cookie is not found, a cooking authentication failed exception will be thrown.
 */
@Component
public class CookieHandler extends AbstractAuthHandler{

    @Override
    public String authType() {
        return AUTH_TYPE_COOKIE;
    }
    ...
    private String generateToken(final Subsystem subsystem) {
        ...
        return CookieHeaderExtractor.extractCookieFromResponse(responseEntity, tokenRef);
    }
}
